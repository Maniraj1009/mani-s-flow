(function () {
  ...
  if (window.__manisflowContentLoaded) {
    console.log("[MANISFLOW] Content script already loaded — skipping duplicate injection.");
    return;
  }
  window.__manisflowContentLoaded = true;

  const LOG_PREFIX = "[MANISFLOW]";
    ...
    if (message?.type === "FLOW_WAIT_GENERATION") {
      ...
      async function getNewUrlsAfterLoad() {
        ...
        for (let attempt = 0; attempt < 5; attempt++) {
          const rawUrls = [...getRealImageUrls(), ...getRealVideoUrls()];
          if (rawUrls.length > 0) return rawUrls.map(url => getTileDataUrl(url));
          await sleep(500);
        }
        const rawUrls = [...getRealImageUrls(), ...getRealVideoUrls()];
        return rawUrls.map(url => getTileDataUrl(url));
      }
